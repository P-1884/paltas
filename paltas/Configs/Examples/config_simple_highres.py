...
config_dict = {
	'main_deflector':{
		...
		'parameters':{
			'M200': 10**(14.5),
			'z_lens': 0.5,
			'gamma': truncnorm(-20,np.inf,loc=2.0,scale=0.0).rvs,
			'theta_E': truncnorm(-1.1/0.15,np.inf,loc=1.1,scale=0.0).rvs,
			'e1': norm(loc=0.0,scale=0.).rvs,
			'e2': norm(loc=0,scale=0.).rvs,
			'center_x': None,
			'center_y': None,
			'gamma1': norm(loc=0.0,scale=0.0).rvs,
			'gamma2': norm(loc=0,scale=0.0).rvs,
			'ra_0':0.0, 'dec_0':0.0
		}
	},
	'source':{
		...
		'parameters':{
			'z_source':truncnorm(-5,np.inf,loc=2.,scale=0.0).rvs,
			'magnitude':uniform(loc=-32,scale=0.0).rvs,
			'output_ab_zeropoint':output_ab_zeropoint,
			'R_sersic':truncnorm(-2,2,loc=0.35,scale=0.0).rvs,
			'n_sersic':truncnorm(3.,np.inf,loc=3.,scale=0.).rvs,
			'e1':norm(loc=0.0,scale=0.0).rvs,
			'e2':norm(loc=0.0,scale=0.).rvs,
			'center_x':norm(loc=centre_loc,scale=0.0).rvs,
		...
		'parameters':{
			'z_source':None,
			'magnitude':truncnorm(-1.5,2.0,loc=-28,scale=0.0).rvs,
			'output_ab_zeropoint':output_ab_zeropoint,
			'R_sersic':truncnorm(-1.333,np.inf,loc=0.8,scale=0.0).rvs,
			'n_sersic':truncnorm(-2.,np.inf,loc=3,scale=0.0).rvs,
			'e1':None,
			'e2':None,
			'center_x':None,
			'center_y':None}
	...
	'detector':{
		'parameters':{
			'pixel_scale':0.005,'ccd_gain':1.58,'read_noise':3.0,
