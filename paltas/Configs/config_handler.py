...
#			self.RSP_coadd_ZP = self.config_module.RSP_coadd_zeropoint
			...
			RSP_image_dict, RSP_exp_dict, RSP_psf_dict, RSP_var_dict = load_RSP(self.RSP_cutout_folder)
			self.RSP_noise_dict = json.load(open(self.RSP_cutout_folder+'/Noise_dict.json'))
			noise_keys = list(self.RSP_noise_dict.keys()).copy()
			for key_i in noise_keys:
				if key_i=='N_central_pixels': continue
				self.RSP_noise_dict[int(key_i)]=self.RSP_noise_dict[key_i]
				del self.RSP_noise_dict[key_i]
	...
	def _draw_image_standard(self,add_noise=True,apply_psf=True,return_noise=False):
		...
		if self.add_RSP_background:
			...
			while resample_RSP:
				...
				N_central_pixels = self.RSP_noise_dict['N_central_pixels']
				N_cutout_pixels = len(RSP_background_cutout)
				pix_start = (N_cutout_pixels-N_central_pixels)//2
				pix_end = (N_cutout_pixels+N_central_pixels)//2
				Aperture_Flux_in_centre = np.sum(RSP_background_cutout[pix_start:pix_end,pix_start:pix_end])
				...
				if (np.sum(np.isnan(RSP_background_cutout))+np.sum(np.isnan(RSP_var))+\
					np.sum(np.isnan(RSP_psf))+np.sum(np.isnan(RSP_N_exp)))==0 and np.sum(RSP_N_exp==0)==0:
					if Aperture_Flux_in_centre<self.RSP_noise_dict[random_RSP_key]:
						resample_RSP=False
					else:
						resample_RSP=True
						print('Resampling due to significant central flux');n_sample+=1
				else: print('Resampling due to nans/zero exposures');n_sample+=1
