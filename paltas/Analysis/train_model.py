...
def save_train_config(model_weights_directory,training_config):
	with open(training_config,'r') as f0:
		with open(model_weights_directory+training_config.split('/')[-1],'w') as f1:
			for line in f0:
				f1.write(line)

def parse_input_args(args_dict):
	class args_class():
		def __init__(self):
			self.training_config = args_dict['training_config']
			self.h5 = args_dict['h5']
			self.tensorboard_dir = args_dict['tensorboard_dir']
	args = args_class()
	return args
...
class PlotLearning(keras.callbacks.Callback):
	...
	def on_epoch_end(self, epoch, logs={}):
		...
		wandb.log({"Loss":self.metrics["loss"][-1],"Val Loss":self.metrics["val_loss"][-1]})
...
def main(args=None,return_data_args=False):
	...
	if not os.path.isdir(f'{config_module.directory_to_save_model}/model_weights/'):
		os.makedirs(f'{config_module.directory_to_save_model}/model_weights/')
	save_train_config(f'{config_module.directory_to_save_model}/model_weights/',args.training_config)
	...
	# The path to the fodler containing the npy images for validation
	...
	wandb.init(
		...
		project="Paltas Training",
		# track hyperparameters and run metadata
        name=config_module.directory_to_save_model.split('paltas_data/')[1].replace('/',''),
		config={
		"Batch Size":batch_size,
		'Learning Rate':learning_rate,
		'Loss Type':loss_function,
		'Norm':norm_images,
		'Training Path':input_norm_path
		})
	...
	if return_data_args: return {'tfr_train_paths':tfr_train_paths,\
								 'tfr_val_path':tfr_val_path,\
								 ...
								 'number_mix':min(batch_size,n_val_npy),\
								 'tf_dataset_t':tf_dataset_t,\
								 'tf_dataset_v':tf_dataset_v,\
								 'params_as_inputs':params_as_inputs,\
								 'params_mix':all_params+log_learning_params}
