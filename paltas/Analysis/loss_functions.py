...
class DiagonalCovarianceLoss(BaseLoss):
	...
	def loss(self,y_true,output):
		...
		Returns:
			(tf.Tensor): The loss function as a tf.Tensor.
		"""
		try:tf.print('output',tf.shape(output))#;tf.print(output)
		except Exception as ex: print('out',ex)
		try:tf.print('y_true',tf.shape(y_true))#;tf.print(y_true)
		except Exception as ex: print('true',ex)
		y_pred, log_var_pred = self.convert_output(output)
		tf.print('y_pred',tf.shape(y_pred),tf.shape(log_var_pred))
		# Add each possible flip to the loss list. We will then take the
		# minimum.
		loss_list = []
		for flip_mat in self.flip_mat_list:
			print('flip_mat',flip_mat)
			loss_list.append(self.log_gauss_diag(y_true,
				tf.matmul(y_pred,flip_mat),log_var_pred))
			# print('loss_list',np.shape(loss_list))
		loss_stack = tf.stack(loss_list,axis=-1)
		tf.print('loss_stack',tf.shape(loss_stack))
		loss_value =  tf.reduce_min(loss_stack,axis=-1)
		tf.print('loss_value',tf.shape(loss_value))
		tf.print('loss value min, max, mean, sum',tf.reduce_min(loss_value),tf.reduce_max(loss_value),tf.reduce_mean(loss_value),tf.reduce_sum(loss_value))
		tf.print('loss value actual',loss_value)
		print('')
		# print('')
		# try:
		# 	tf.print('log_var_pred',log_var_pred.shape)
		# 	print('log_var_pred min+max')
		# 	tf.print(tf.math.reduce_min(log_var_pred))
		# 	tf.print(tf.math.reduce_max(log_var_pred))
		# except Exception as ex: print(ex)
		# try:
		# 	tf.print('loss_stack',loss_stack.shape)
		# 	print('loss_stack min+max')
		# 	tf.print(tf.math.reduce_min(loss_stack))
		# 	tf.print(tf.math.reduce_max(loss_stack))
		# except Exception as ex: print(ex)
		# try:
		# 	tf.print('y_pred',y_pred.shape)
		# 	print('y_pred min+max')
		# 	tf.print(tf.math.reduce_min(y_pred,axis=0))
		# 	tf.print(tf.math.reduce_max(y_pred,axis=0))
		# except Exception as ex: print(ex)
		# try:
		# 	tf.print('output',output.shape)
		# 	print('output min+max')
		# 	tf.print(tf.math.reduce_min(output,axis=0))
		# 	tf.print(tf.math.reduce_max(output,axis=0))
		# except Exception as ex: print(ex)
		# try:
		# 	tf.print('y_true',y_true.shape)
		# 	print('y_true min+max')
		# 	tf.print(tf.math.reduce_min(y_true,axis=0))
		# 	tf.print(tf.math.reduce_max(y_true,axis=0))
		# except Exception as ex: print(ex)
		# try:
		# 	tf.print('loss_value',loss_value.shape,tf.math.reduce_min(loss_value),tf.math.reduce_max(loss_value))
		# except Exception as ex: print(ex)
		return loss_value
