...
def load_RSP_legacy(folder_name):
	...
	return image_dict, exp_dict, psf_dict, var_dict

def load_RSP_multiprocess_file(filename_i):
	print(filename_i)
	exp_file = filename_i.replace('image_data','Nexp_data')
	psf_file = filename_i.replace('image_data','psf_data')
	var_file = filename_i.replace('image_data','var_data')        
	...
	with h5py.File(var_file,'r') as v0:
		var_i = v0['data'][()]
	return filename_i,image_i,exp_i,psf_i,var_i
...
def load_RSP(folder_name):
	...
	var_dict = {}
	with Pool() as p:
		results = list(tqdm(p.imap(load_RSP_multiprocess_file,file_names),total=len(file_names)))
	for filename_i,image_i,exp_i,psf_i,var_i in results:
		key_i = int(filename_i.replace('/mnt/extraspace/hollowayp/paltas_data/','').replace('Coadd_files','').replace('_good_seeing','').replace('_testset','').replace('.h5','').replace('_image_data','').replace('/','').replace('.',''))
		image_dict[key_i] = image_i
		exp_dict[key_i] = exp_i
		psf_dict[key_i] = psf_i
		var_dict[key_i] = var_i
	return image_dict, exp_dict, psf_dict, var_dict
