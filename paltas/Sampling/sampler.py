...
#		print('draw dict',draw_dict)
	...
	def sample(self):
		...
		try:
			assert 'd_center_x' in full_param_dict['source_parameters'].keys() and \
				   'd_center_y' in full_param_dict['source_parameters'].keys()
			# print('Initial',full_param_dict)
			full_param_dict['source_parameters']['center_x'] = full_param_dict['source_parameters']['d_center_x']+\
																full_param_dict['main_deflector_parameters']['center_x']
			full_param_dict['source_parameters']['center_y'] = full_param_dict['source_parameters']['d_center_y']+\
																full_param_dict['main_deflector_parameters']['center_y']
			# print('full_param_dict,center_x',full_param_dict['source_parameters']['center_x'])
			# print('full_param_dict,center_y',full_param_dict['source_parameters']['center_y'])
			del full_param_dict['source_parameters']['d_center_x']
			del full_param_dict['source_parameters']['d_center_y']
			# #Updating config_dict:
			# self.config_dict['source']['parameters']['center_x'] = full_param_dict['source_parameters']['center_x']
			# self.config_dict['source']['parameters']['center_y'] = full_param_dict['source_parameters']['center_y']
			# print('config dict,center_x',self.config_dict['source']['parameters']['center_x'])
			# print('config dict,center_y',self.config_dict['source']['parameters']['center_y'])
			# del self.config_dict['source']['parameters']['d_center_x']
			# del self.config_dict['source']['parameters']['d_center_y']
		except Exception as ex:
			assert 'center_x' in full_param_dict['source_parameters'].keys() and \
				   'center_y' in full_param_dict['source_parameters'].keys()
	...
	def get_sample_dist(self):
		...
		assert 'd_center_x' not in full_param_dict['source_parameters'].keys() and \
				'd_center_y' not in full_param_dict['source_parameters'].keys() #Not yet set up here
		return full_param_dict
	

	def catalog_sample(self, index):
		try:
			self.initialised #This function is called once more than the number of images requested, hence have to have an 'initialised' boolean, so the index used for initialisation can be re-used when actually generating the images.    
		except:
		...
		if index is None:
			...
			if index in self.selected_indices:
				while index in self.selected_indices:
					#This prevents indices being reused if they have already been used to generate an image:
					index = np.random.choice(list(set(self.deflectors_catalog.index) - set(self.selected_indices)))
		else:
			index = index
		if self.initialised: self.selected_indices.append(index)
		# print('index: ', index)
