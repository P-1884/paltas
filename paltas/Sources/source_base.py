...
class SourceBase():
	...
	def check_parameterization(self, required_params):
		...
		if not all(elem in self.source_parameters.keys() for elem in ['center_x','center_y']) and \
		   not all(elem in self.source_parameters.keys() for elem in ['d_center_x','d_center_y']): 
				raise ValueError('Need source position parameters')
