...
def plot_collage(image_folder,N_rows=2,N_cols=2,vmin=None,vmax=None,stretch=None,interval=None,\
                 figsize=None,tight_layout=False,title=None,fontsize=18,h5_filename = None,cmap=None,
                 return_fig = False,ax=None,seed=1,saveas = None):
    ...
    image_fits_list = glob.glob(f'{image_folder}/*.fits')
    ...
    if len(image_jpeg_list)==0:
        image_jpeg_list = glob.glob(f'{image_folder}/*.jpg')
    ...
    elif len(image_fits_list)>0:
        image_list = [fits.open(image_fits_list[n_im])[0].data for n_im in range(N_images)]
    ...
    if saveas is not None: pl.savefig(saveas)
