...
#
     ...
     def write_new_config(N_values_changed):
          begin_config=False
          key_tracker = []
...
#                    print('LINE',line)
                    ...
                    if begin_config:
                         ...
                         try:
                              ...
                              pass
                    if 'config_dict' in line: begin_config=True
          return N_values_changed     
     N_values_changed = write_new_config(N_values_changed)
     def add_on_cross_object(list_of_properties,list_of_means,list_of_std,redshift=True,position=True,shear=True,N_values_changed=np.nan):
          '''
          ...
          with open(new_config_file_path,'w') as f_new:
                ...
                shear_indx = np.where(list_of_properties=='main_deflector*parameters*shear')[0];assert len(shear_indx)==1
                ...
                shear_mean = float(np.array(list_of_means)[shear_indx])
                shear_std = float(np.array(list_of_std)[shear_indx])
                ...
                if shear:
                        f_new.write(f"'main_deflector:gamma1,main_deflector:gamma2':dist.ExternalShearTranslation(\n"+\
                            f"gamma_dist=norm(loc={shear_mean},scale={shear_std}).rvs, \n"+\
                            f"phi_dist=uniform(loc=0,scale=2*np.pi).rvs),\n")
                        N_values_changed+=1
                f_new.write("}\n")
                f_new.write("}\n")
                f_new.write("}")
     ...
     if add_crossmatch: N_values_changed = add_on_cross_object(list_of_properties,list_of_means,list_of_std,N_values_changed=N_values_changed)
     try:
        assert N_values_changed==N_new_values #Assert that the same number of value have been updated as in the new_config_dict_values dictionary.  
     except Exception as ex1:
        print(ex1)
        print('N_values_changed',N_values_changed)
        print('N_new_values',N_new_values)
        print('list_of_properties',list_of_properties)
        print('new config',new_config_dict_values)